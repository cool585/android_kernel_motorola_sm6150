					...
					int forced_base)
{
	...
	for (i = 0; i < prop->len; i++) {
		...
		if (kstrtoint(tmp, base, &val)) {
			pr_debug_once("error converting %s at %d\n",
				tmp, i);
			...
			const char *propname, u32 *out_value)
{
	...
	if (rc) {
		pr_debug_once("prop=%s error(%d) converting %s, base=%d\n",
			      ...
			      u32 *out_values, size_t sz)
{
	...
	for (i = 0; i < prop->len; i++) {
		...
		if (rc) {
			pr_debug_once("prop=%s error(%d) converting %s(%d), base=%d\n",
				...
				int index, const char **output)
{
	...
	if (prop->type != DSI_PROP_TYPE_STR_ARRAY) {
		pr_debug_once("not a string array property\n");
		goto end;
	}

	if (index >= prop->len) {
		pr_debug_once("out of bond index %d\n", index);
		goto end;
	}
				...
				const u8 *data, u32 size)
{
	...
	if (!parser->head_node) {
		pr_debug_once("could not get head node\n");
				...
				const u8 **buf, u32 *size)
{
	...
	if (rc || !parser->fw) {
		pr_debug_once("couldn't read firmware\n");
		goto end;
	}
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%31s", parser->file_name) != 1) {
		pr_debug_once("failed to get val\n");
		goto end;
	}

	if (dsi_parser_read_file(parser, &data, &size)) {
		pr_debug_once("failed to read file\n");
		goto end;
	}
	...
	if (!parser->head_node) {
		pr_debug_once("failed to parse data\n");
		goto end;
	}
end:
	return len;
...
int dsi_parser_dbg_init(void *parser, struct dentry *parent_dir)
{
	...
	if (IS_ERR_OR_NULL(dir)) {
		...
		pr_debug_once("failed to create parser debugfs\n");
		goto end;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		...
		pr_debug_once("failed to create init debugfs\n");
		goto dbg;
	}
